...
# Même chose par groupe: 
# Sol 1: 
t.test(Prewt[Treat == "CBT"], 
       Postwt[Treat == "CBT"], 
       paired=TRUE,data=anorexia)
t.test(Prewt[Treat == "Cont"],
       Postwt[Treat == "Cont"],
       paired=TRUE,data=anorexia)
t.test(Prewt[Treat == "FT"],
       Postwt[Treat == "FT"], 
       paired=TRUE,data=anorexia)
# Sol 2 (diff=0)
d= Postwt - Prewt
t.test(d[Treat == "CBT"],data=anorexia)
...
d = Postwt - Prewt
t.test(d[Treat == "Cont"],
       d[Treat == "CBT"],data=anorexia)
t.test(d[Treat == "Cont"],
       d[Treat == "CBT"],data=anorexia,var.equal=T)
# --------------------------------------------------------------
...
# -------------------------------------------------------------- 
NewTreat = as.factor(ifelse(Treat=="Cont",
                            "A",
                            ifelse(Treat=="CBT","B","C")))
...
#         Prewt augmentation pour entre 2 patients pour une difference initiale de 1 kg 
# -------------------------------------------------------------- 
NewTreat=as.factor(ifelse(Treat=="Cont","A",ifelse(Treat=="CBT","B","C")))
